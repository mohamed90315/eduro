...
const String monthlyApiUrl = "https://mock-api.net/api/Flutter/monthlyTasks";
const String weeklyApiUrl = "https://mock-api.net/api/Flutter/weeklyTasks";
...
class _RoadmapPageState extends State<RoadmapPage> {
  ...
  List<RoadmapTask> _weeklyTasks = [];
  bool _isLoading = true;

  Future<void> _fetchTasks() async {
    try {
      final monthlyResponse = await http.get(Uri.parse(monthlyApiUrl));
      final weeklyResponse = await http.get(Uri.parse(weeklyApiUrl));

      if (monthlyResponse.statusCode == 200 && weeklyResponse.statusCode == 200) {
        final monthlyData = jsonDecode(monthlyResponse.body) as List;
        final weeklyData = jsonDecode(weeklyResponse.body) as List;

        setState(() {
          _tasks = monthlyData.map((e) => RoadmapTask.fromJson(e)).toList();
          _weeklyTasks = weeklyData.map((e) => RoadmapTask.fromJson(e)).toList();
          _isLoading = false;
        });
      } else {
...
class RoadmapTask {
  ...
  // JSON factory (so your API can still work)
  factory RoadmapTask.fromJson(Map<String, dynamic> json) {
    return RoadmapTask(
      ...
      category: TaskCategory.values.firstWhere(
            (e) => e.toString().split('.').last == (json['category'] ?? "other"),
