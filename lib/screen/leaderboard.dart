...
class UserScore {
  ...
  factory UserScore.fromJson(Map<String, dynamic> json) {
    return UserScore(
      ...
      scoreAllTime: json['scoreAllTime'],
      scoreMonthly: json['scoreMonthly'],
    );
  }
...
class MockRepo {
  ...
  static List<Team> initialUserTeams() {
    final team = Team(
      ...
      inviteCode: 'EGY1',
      members: [
        UserScore(username: 'Jerry', scoreAllTime: 18377, scoreMonthly: 4000),
        UserScore(username: 'Omar', scoreAllTime: 27233, scoreMonthly: 6800),
        UserScore(username: 'Alice', scoreAllTime: 34741, scoreMonthly: 8200),
        UserScore(username: 'Bob', scoreAllTime: 26635, scoreMonthly: 6400),
        UserScore(username: 'Hiro', scoreAllTime: 22902, scoreMonthly: 7200),
        UserScore(username: 'Mika', scoreAllTime: 9188, scoreMonthly: 1900),
        UserScore(username: 'Carlos', scoreAllTime: 20439, scoreMonthly: 5400),
        UserScore(username: 'Fernanda', scoreAllTime: 10439, scoreMonthly: 1400),
      ],
    );
...
class _LeaderboardPageState extends State<LeaderboardPage> {
  ...
  Future<void> _loadTeams() async {
    ...
    if (stored != null) {
      ...
      userTeams = await TeamService.fetchTeams();
    }
  ...
  Future<void> _handleJoinTeam() async {
    ...
    try {
      // ✅ Fetch all teams from mock API
      final allTeams = await TeamService.fetchTeams();

      // ✅ Find team by invite code (no null-safety needed anymore)
      final team = allTeams.firstWhere(
            (t) => t.inviteCode == code,
        orElse: () => throw Exception("Invalid team code"),
      );


      // ✅ Add to user's teams if not already joined
  ...
  Future<void> _handleCreateTeam() async {
    ...
    try {
      // Create team object with user input
      final newTeam = Team(
        id: "", // leave empty, API should assign ID
        name: name,
        inviteCode: "INV-${DateTime.now().millisecondsSinceEpoch}", // simple generator
        members: [],
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Team "${createdTeam.name}" created. Code: ${createdTeam.inviteCode}',
          ),
        ),
      );
    } catch (e) {
            ...
            children: [
              ...
              _buildFloatingActionTile(
                ...
                onTap: () async {
                  ...
                  try {
                    await TeamService.leaveTeam(team.id, "currentUsername"); // TODO: replace with actual logged-in user
                    setState(() {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('You left "${team.name}".')),
                    );
                  } catch (e) {
                    ScaffoldMessenger.of(context).showSnackBar(
              ...
              _buildFloatingActionTile(
                ...
                onTap: () async {
                  ...
                  try {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('Team "${team.name}" deleted.')),
                    );
                  } catch (e) {
                    ScaffoldMessenger.of(context).showSnackBar(
...
// Edit name
              _buildFloatingActionTile(
                ...
                onTap: () async {
                  ...
                  if (newName != null && newName.trim().isNotEmpty) {
                    try {
                      final updatedTeam = await TeamService.editTeam(
                        team.id,
                        {"name": newName.trim()},
                      );

                      setState(() {
                        team.name = updatedTeam.name; // sync with API response
                      });

                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Team renamed to "${updatedTeam.name}".')),
                      );
                    } catch (e) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Error renaming team: $e')),
                      );
                    }
                  }
              ...
              _buildFloatingActionTile(
                ctx,
                icon: Icons.code,
                text: 'Team code: ${team?.inviteCode ?? "N/A"}',
                onTap: () {}, // no action
              ),
            ],
          ),
